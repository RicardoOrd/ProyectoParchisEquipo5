...
// En TableroUI.java
    // Método modificado para recibir el avatar dinámicamente// En TableroUI.java
    public void iniciarCliente(String host, String nickname, String avatar) {
        this.clienteRed = new Cliente();
        this.clienteRed.setTableroView(this);

        // CORRECCIÓN: Agregamos el 5to parámetro ("") que es el código de sala
        this.clienteRed.conectar(host, 5000, nickname, avatar, "");

        this.setTitle("Parchís - Jugador: " + nickname + " (" + avatar + ")");
    }
    ...
    public void procesarMensajeJuego(String json) {
        if (json.contains("DICE_RESULT")) {
            ...
            // Si el mensaje es de turno, actualizamos la etiqueta gigante
            if (msg.startsWith("Turno de")) {
                lblTurno.setText(msg);

                // Opcional: Cambiar color según el texto (si quieres un extra visual)
                if (msg.contains("AMARILLO")) {
                    lblTurno.setForeground(Color.YELLOW);
                } else if (msg.contains("AZUL")) {
                    lblTurno.setForeground(Color.BLUE);
                } else if (msg.contains("ROJO")) {
                    lblTurno.setForeground(Color.RED);
                } else if (msg.contains("VERDE")) {
            ...
            try {
                String ganador = json.split("\"winner\": \"")[1].split("\"")[0];

                // Mostramos el mensaje (esto detiene la ejecución hasta que le den OK)
                JOptionPane.showMessageDialog(this, "¡Felicidades " + ganador + "! Has ganado la partida.");

                // Cerramos la ventana del juego
                this.dispose();

                // Abrimos el menú principal de nuevo
                new MenuUI().setVisible(true);

            } catch (Exception e) {
                System.out.println("Error al procesar victoria: " + e.getMessage());
            }
        }
    }
...
// Modifica este método
    public void mostrarResultadoDado(int valorFinal, Color color) {
        // Deshabilitar botón mientras gira
        btnLanzarDado.setEnabled(false);

        // Timer para la animación (cambia cada 100ms)
        Timer timerAnimacion = new Timer(100, null);

        // Variable auxiliar para contar vueltas de animación
        final int[] vueltas = {0};

        timerAnimacion.addActionListener(e -> {
            // Mostrar número aleatorio mientras "gira"
            int valorRandom = (int) (Math.random() * 6) + 1;
            lblDado.setText("🎲 " + valorRandom);
            lblDado.setForeground(Color.GRAY); // Color temporal

            vueltas[0]++;
            if (vueltas[0] > 10) { // Después de 10 cambios (1 segundo)
                timerAnimacion.stop();
                // Mostrar el valor REAL que vino del servidor
                lblDado.setText("🎲 " + valorFinal);
                lblDado.setForeground(color);
                btnLanzarDado.setEnabled(true); // Reactivar botón
            }
        });
